...
-- 通用工具：為 TextLabel / TextButton 加上 TextScaled 自動字體縮放
local function enableAutoTextScale(textObject, minSize, maxSize)
...
function UIModule.CreateWindow(titleText, footerText)
    ...
    -- 標題自動縮放
    enableAutoTextScale(Title, 8, 16)
    ...
    Content.ScrollBarThickness = 0 -- 隱藏滾動條
    ...
    UIList.Padding = UDim.new(0, 6) -- 控制組件間的垂直距
    UIList.HorizontalAlignment = Enum.HorizontalAlignment.Center -- 居中對齊
    self.UIList = UIList
    ...
    if footerText then
        ...
        enableAutoTextScale(Footer, 6, 11)
...
function UIModule:CreateButton(txt, cb)
    ...
    -- 加入內邊距 Padding，避免文字直接貼到按鈕邊緣
    local UIPadding = Instance.new("UIPadding")
    UIPadding.PaddingLeft = UDim.new(0, 6)
    UIPadding.PaddingRight = UDim.new(0, 6)
    UIPadding.Parent = btn

    -- 自動縮放文字 (最小 6pt，最大 12pt)
    enableAutoTextScale(btn, 6, 12)
...
function UIModule:CreateToggle(txt, defaultState, cb)
    ...
    btn.Size = UDim2.new(0.92, 0, 0, 32)
    btn.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    btn.Text = "" -- 按鈕本體不顯示文字，改由獨立 TextLabel 控制以防覆蓋右側開關盒
    btn.Parent = self.Content
    ...
    label.Parent = btn
    enableAutoTextScale(label, 6, 12)
...
function UIModule:CreateSlider(txt, min, max, default, cb)
    ...
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.Parent = frame
    enableAutoTextScale(label, 6, 11)
...
function UIModule:CreateDropdown(txt, options, default, cb)
    ...
    enableAutoTextScale(mainBtn, 6, 12)
    ...
    for _, optionText in ipairs(options) do
        ...
        enableAutoTextScale(optBtn, 6, 11)
...
function UIModule:CreateInput(txt, placeholder, cb)
    ...
    enableAutoTextScale(textBox, 6, 11)
