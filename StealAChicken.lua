...
local autoCollectEggEnabled = false -- 新增：自動收蛋開關
...
-- 4. 自動收蛋 Toggle (新增)
Hub:CreateToggle("Auto Collect Egg", false, function(isOn)
    autoCollectEggEnabled = isOn

    if autoCollectEggEnabled then
        task.spawn(function()
            while autoCollectEggEnabled do
                -- 優先動態搜尋 Remote
                local claimRemote = getRemote("data.base.claimAllEggs")
                ...
                if claimRemote then
                    ...
                    -- Fallback: 使用寫死的路徑（防止動態搜尋異常）
                    pcall(function()
                        local fallbackEvent = ReplicatedStorage.packages._Index["littensy_remo@1.5.3"].remo.container["data.base.claimAllEggs"]
